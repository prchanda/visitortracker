...
(function($, window, undefined) {
        ...
        configureStopReconnectingTimeout = function(connection) {
            ...
            if (!connection._.configuredStopReconnectingTimeout) {
                onReconnectTimeout = function(connection) {
                    connection.log("Couldn't reconnect within the configured timeout (" +
    ...
    function validateTransport(requestedTransport, connection) {
        ...
        if ($.isArray(requestedTransport)) {
            ...
            for (var i = requestedTransport.length - 1; i >= 0; i--) {
                ...
                if ($.type(requestedTransport) !== "object" &&
                    ($.type(transport) !== "string" || !signalR.transports[transport])) {
        ...
        } else if ($.type(requestedTransport) !== "object" &&
    ...
    signalR.fn = signalR.prototype = {
        ...
        transportConnectTimeout:
            0, // This will be modified by the server in respone to the negotiate request.  It will add any value sent down from the server to the client value.
                                ...
                                $.each(config.transport,
                                    function() {
                                        ...
                                        ($.type(transport) === "string" &&
                                            $.inArray("" + transport, supportedTransports) >= 0)) {
                                            transports.push($.type(transport) === "string"
                                                ? "" + transport
                                                : transport);
                                        }
                                    });
                        ...
                        $.signalR.ajaxDefaults,
                        {
                            ...
                            success: function(result) {
                                ...
                                if (data.Response === "pong") {
                                    ...
                                    deferral.reject(
                                        signalR._.transportError(
                                            signalR._.format(signalR.resources.pingServerFailedInvalidResponse,
                                                result.responseText),
                                            connection.transport
                                        )
                                    );
                            ...
                            error: function(error) {
                                if (error.status === 401 || error.status === 403) {
                                    deferral.reject(
                                        signalR._.transportError(
                                            signalR._.format(signalR.resources.pingServerFailedStatusCode,
                                                error.status),
                                            connection.transport,
                                            error
                                        )
        ...
        ajaxSend: function(connection, data) {
            var payload = transportLogic.stringifySend(connection, data),
                url = connection.url +
                    ...
                    "&connectionToken=" +
                    window.encodeURIComponent(connection.token),
                onFail = function(error, connection) {
                    $(connection).triggerHandler(events.onError,
                        [signalR._.transportError(signalR.resources.sendFailed, connection.transport, error), data]);
                };
            ...
            connection.eventSource.addEventListener("error",
                function(e) {
                    ...
                    if (e.eventPhase === window.EventSource.CLOSED) {
                        ...
                        $connection.triggerHandler(events.onError,
                            [signalR._.transportError(signalR.resources.eventSourceError, connection.transport, e)]);
                    }
                },
                false);
        },
                    ...
                    if (signalR._.ieVersion <= 8) {
                        ...
                        if (attachedTo === 0) {
                            ...
                            loadingFixIntervalId = window.setInterval(function() {
                                    var tempFrame =
                                        $(
                                            "<iframe style='position:absolute;top:0;left:0;width:0;height:0;visibility:hidden;' src=''></iframe>");
    ...
    signalR.transports.foreverFrame = {
        ...
        start: function(connection, onSuccess, onFailed) {
            var that = this,
                ...
                url,
                frame = $("<iframe data-signalr-connection-id='" +
                    connection.id +
                    "' style='position:absolute;top:0;left:0;width:0;height:0;visibility:hidden;' src=''></iframe>");
            ...
            frame.bind("readystatechange",
                function() {
                    if ($.inArray(this.readyState, ["loaded", "complete"]) >= 0) {
                        connection.log("Forever frame iframe readyState changed to " +
                            this.readyState +
                            ", reconnecting.");
            ...
            if (connection.frame) {
                if (connection.frame.stop) {
                    ...
                    try {
                        ...
                        connection.log("Error occured when stopping foreverFrame transport. Message = " +
                            e.message +
                            ".");
                    }
                }
                                        ...
                                        if (!tryFailConnect()) {
                                            ...
                                            if (connection.state !== signalR.connectionState.reconnecting) {
                                                connection.log("An error occurred using longPolling. Status = " +
                                                    ...
                                                    data.responseText +
                                                    ".");
                                                $(instance).triggerHandler(events.onError,
                                                    [
                                                        signalR._.transportError(signalR.resources.longPollFailed,
                                                            connection.transport,
                                                            data)
                                                    ]);
                                            }
